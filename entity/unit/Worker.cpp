...
Worker::Worker(QPointF pos, Unit::Type type, Player *player, ResourceManager *rm) : Unit(pos, type, 0.85f, 0, 0, 0, 0) {
    ...
    case PEASANT:
        spriteSheet = rm->getSprite("PEASANT");
        spriteSheetFlipped = rm->getSprite("PEASANT_flipped");
        break;
    case PEON:
        spriteSheet = rm->getSprite("PEON");
        spriteSheetFlipped = rm->getSprite("PEON_flipped");
        break;
    }
